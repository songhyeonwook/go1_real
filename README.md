...
python3 deploy.py --mode walk --policy model/P3-final/exported/policy_numpy.npz \
    --vx 0.3 --duration 10 --power-protect 9 --log-npz walk1.npz
```

속도는 0.3 으로 시작해 안정되면 0.4 까지 올립니다 (아래 실기 기록 참고).

`--power-protect` 의 기본값 5 는 SDK 가 모터 출력을 50 % 로 제한합니다. 정책이
학습에서 쓴 토크를 못 내면 다리가 처지고 몸통이 앞뒤로 흔들립니다 (2026-09-17 실기:
level 5, vx 0.2 에서 pitch −0.12 → +0.16 rad, contact 4→2 로 흔들려 중단). 검증된
보행 기록은 전부 `--power-protect 9` 였고 그때 토크 피크는 한계의 68 % 였습니다.
`--log-npz` 는 항상 붙이세요 — 넘어지거나 중단해도 저장됩니다.
...
* **추정치는 평활해서 보세요.** 50 Hz 순간값은 출렁이고, 인계 직후에는 일시적인 가짜
  스파이크가 나옵니다. 텔레메트리의 `[EST]` 줄은 보조 헤드(`aux_heads.npz`) 출력으로
  **제어에는 쓰이지 않는 진단값**입니다. `v_hat` 은 1초 EMA 로 평활되고, 정책 시작 후
  1초 동안은 `(수렴 중)` 이 붙습니다. `L_hat`(부목 길이)은 `--injured-leg` 일 때만
  표시합니다 — 그 헤드는 부목 길이 0.3~0.4 m 라벨로만 학습돼서 정상 로봇에서는
  평균값 0.35 m 를 그냥 출력할 뿐 아무 의미가 없습니다.
